...
class DisplayMetrics(object):
    ...
    def __call__(self):
        ...
        # Build selection arrays and cross references ([[hosts], [plugins], [mgids]] used to interpret
        # the h2 parameter and passed to chameleon to build the HTML lists.
        Hosts = {}
        Plugins = {}
        Xref = [[], [], []]

        for host in sorted(PluginConfigs['links']):
            Hosts[host] = {'checked': False, 'plugins': {}}

            if not host in Xref[0]:
                Xref[0] += [host]

            for plugin in sorted(PluginConfigs['links'][host]):
                Hosts[host]['plugins'][plugin] = {'checked': False, 'mgids': {}}
                Plugins[plugin] = {'hosts': [], 'mgids': {}}

                if not host in Plugins[plugin]['hosts']:
                    Plugins[plugin]['hosts'] += [host]

                if not plugin in Xref[1]:
                    Xref[1] += [plugin]
                ...
                for mgid in sorted(PluginConfigs['config'][PluginConfigs['links'][host][plugin]]):
                    ...
                    Plugins[plugin]['mgids'][mgid] = []

                    if not host in Plugins[plugin]['mgids'][mgid]:
                        Plugins[plugin]['mgids'][mgid] += [host]

                    if not mgid in Xref[2]:
                        Xref[2] += [mgid]
        ...
        for h2_item in Options['h2']['value'].split('_'):
            if h2_item != 's2':
                ...
                for ix in range(len(sel)):
                    sel[ix] = MCutils.StrToInt(sel[ix])
                    if sel[ix] < 0 or sel[ix] >= len(Xref[ix]):
                        h2_parm_valid = False
                        break
        ...
        # Scan selections array to generate selected graphs/CSVs.
        Selections = []
